...
def write_openmrs_id_to_database(openmrs_uuid, opensrp_uuid):
	if test_run == False:
		cur = conn.cursor()
		sql = ("update mergeset set openmrs_id = '{0}' where opensrp_id = '{1}'").format(openmrs_uuid, opensrp_uuid)
		logging.debug(sql)
		cur.execute(sql)
		conn.commit()
...
def update_database(opensrp_uuid):
	if test_run == False:
		dt = datetime.now()
		cur = conn.cursor()
		sql = ("update mergeset set processed_date= '{1}', processed = true where opensrp_id = '{0}'").format(opensrp_uuid,dt)
		logging.debug(sql)
		cur.execute(sql)
		conn.commit()
	...
	for l in locations: 
		...
		if location['operation']=='i' and location['processed'] != True:
			...
			if test_run == False:
				...
				logging.info('{0}/{1} Test Run reveal insert'.format(locations_progress,locations_total))
				openMRS_id = 'test'		
		#update db with openMRS id
			...
			if location['d_name'] == True:
				...
				mrs = json.loads(c.text)
				logging.debug(mrs)
				logging.info('{0} & {1} change to {2}'.format(mrs['name'],mrs['name'],location['name_local']))
				...
				if test_run == False:
					...
					logging.info('OpenMRS post response code: {0}'.format(c.status_code))
					logging.debug('OpenMRS post response: {0}'.format(c.text))
				else:
					logging.info('{0}/{1} Test Run omrs update'.format(locations_progress,locations_total))
			...
			if test_run == False:
				...
				logging.info('{0}/{1} Test Run reveal update'.format(locations_progress,locations_total))
